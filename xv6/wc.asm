...
wc(int fd, char *name)
{
  ...
  a6:	68 0a 07 00 00       	push   $0x70a
  ab:	6a 01                	push   $0x1
  ad:	e8 89 03 00 00       	call   43b <printf>
}
  ...
  c0:	68 fa 06 00 00       	push   $0x6fa
  c5:	6a 01                	push   $0x1
  c7:	e8 6f 03 00 00       	call   43b <printf>
    exit();
...
main(int argc, char *argv[])
{
 ...
 14c:	ff 37                	pushl  (%edi)
 14e:	68 17 07 00 00       	push   $0x717
 153:	6a 01                	push   $0x1
 155:	e8 e1 02 00 00       	call   43b <printf>
      exit();
...
putc(int fd, char c)
{
 ...
 3a7:	88 55 f4             	mov    %dl,-0xc(%ebp)
  write(fd, &c, 1);
 3aa:	6a 01                	push   $0x1
 3ac:	8d 55 f4             	lea    -0xc(%ebp),%edx
 3af:	52                   	push   %edx
 3b0:	50                   	push   %eax
 3b1:	e8 5b ff ff ff       	call   311 <write>
}
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 3ca:	0f 95 c3             	setne  %bl
 3cd:	89 d0                	mov    %edx,%eax
 3cf:	c1 e8 1f             	shr    $0x1f,%eax
 3d2:	84 c3                	test   %al,%bl
 3d4:	74 10                	je     3e6 <printint+0x2b>
    neg = 1;
    x = -xx;
 3d6:	f7 da                	neg    %edx
    neg = 1;
 ...
 3e4:	eb 0b                	jmp    3f1 <printint+0x36>
  neg = 0;
 3e6:	c7 45 d4 00 00 00 00 	movl   $0x0,-0x2c(%ebp)
 3ed:	eb f0                	jmp    3df <printint+0x24>
  do{
    buf[i++] = digits[x % base];
 ...
 3fc:	8d 46 01             	lea    0x1(%esi),%eax
 3ff:	0f b6 92 34 07 00 00 	movzbl 0x734(%edx),%edx
 406:	88 54 35 d8          	mov    %dl,-0x28(%ebp,%esi,1)
  }while((x /= base) != 0);
 40a:	89 da                	mov    %ebx,%edx
 40c:	85 db                	test   %ebx,%ebx
 40e:	75 df                	jne    3ef <printint+0x34>
 410:	89 c3                	mov    %eax,%ebx
  if(neg)
 412:	83 7d d4 00          	cmpl   $0x0,-0x2c(%ebp)
 416:	74 16                	je     42e <printint+0x73>
    buf[i++] = '-';
 418:	c6 44 05 d8 2d       	movb   $0x2d,-0x28(%ebp,%eax,1)
 41d:	8d 5e 02             	lea    0x2(%esi),%ebx
 420:	eb 0c                	jmp    42e <printint+0x73>

  while(--i >= 0)
    putc(fd, buf[i]);
 422:	0f be 54 1d d8       	movsbl -0x28(%ebp,%ebx,1),%edx
 427:	89 f8                	mov    %edi,%eax
 429:	e8 73 ff ff ff       	call   3a1 <putc>
  while(--i >= 0)
 42e:	83 eb 01             	sub    $0x1,%ebx
 431:	79 ef                	jns    422 <printint+0x67>
}
 433:	83 c4 2c             	add    $0x2c,%esp
 436:	5b                   	pop    %ebx
 437:	5e                   	pop    %esi
 438:	5f                   	pop    %edi
 439:	5d                   	pop    %ebp
...
printf(int fd, const char *fmt, ...)
{
 ...
 45b:	e8 41 ff ff ff       	call   3a1 <putc>
 ...
 473:	0f 84 23 01 00 00    	je     59c <printf+0x161>
    c = fmt[i] & 0xff;
 ...
 48a:	eb db                	jmp    467 <printf+0x2c>
      if(c == 'd'){
 48c:	83 f8 64             	cmp    $0x64,%eax
 48f:	74 49                	je     4da <printf+0x9f>
        printint(fd, *ap, 10, 1);
        ap++;
      } else if(c == 'x' || c == 'p'){
 491:	83 f8 78             	cmp    $0x78,%eax
 494:	0f 94 c1             	sete   %cl
 497:	83 f8 70             	cmp    $0x70,%eax
 49a:	0f 94 c2             	sete   %dl
 49d:	08 d1                	or     %dl,%cl
 49f:	75 63                	jne    504 <printf+0xc9>
        printint(fd, *ap, 16, 0);
        ap++;
      } else if(c == 's'){
 4a1:	83 f8 73             	cmp    $0x73,%eax
 4a4:	0f 84 84 00 00 00    	je     52e <printf+0xf3>
          s = "(null)";
        while(*s != 0){
          putc(fd, *s);
          s++;
 ...
 4ad:	0f 84 b7 00 00 00    	je     56a <printf+0x12f>
        putc(fd, *ap);
        ap++;
      } else if(c == '%'){
 4b3:	83 f8 25             	cmp    $0x25,%eax
 4b6:	0f 84 cc 00 00 00    	je     588 <printf+0x14d>
        putc(fd, c);
      } else {
 ...
 4c4:	e8 d8 fe ff ff       	call   3a1 <putc>
        putc(fd, c);
 4c9:	89 fa                	mov    %edi,%edx
 4cb:	8b 45 08             	mov    0x8(%ebp),%eax
 4ce:	e8 ce fe ff ff       	call   3a1 <putc>
      }
      state = 0;
 4d3:	be 00 00 00 00       	mov    $0x0,%esi
 4d8:	eb 8d                	jmp    467 <printf+0x2c>
        printint(fd, *ap, 10, 1);
 4da:	8b 7d e4             	mov    -0x1c(%ebp),%edi
 4dd:	8b 17                	mov    (%edi),%edx
 4df:	83 ec 0c             	sub    $0xc,%esp
 4e2:	6a 01                	push   $0x1
 4e4:	b9 0a 00 00 00       	mov    $0xa,%ecx
 4e9:	8b 45 08             	mov    0x8(%ebp),%eax
 4ec:	e8 ca fe ff ff       	call   3bb <printint>
        ap++;
 ...
 4ff:	e9 63 ff ff ff       	jmp    467 <printf+0x2c>
        printint(fd, *ap, 16, 0);
 ...
 516:	e8 a0 fe ff ff       	call   3bb <printint>
        ap++;
 ...
 529:	e9 39 ff ff ff       	jmp    467 <printf+0x2c>
        s = (char*)*ap;
 ...
 53b:	75 28                	jne    565 <printf+0x12a>
          s = "(null)";
 53d:	be 2b 07 00 00       	mov    $0x72b,%esi
 542:	8b 7d 08             	mov    0x8(%ebp),%edi
 545:	eb 0d                	jmp    554 <printf+0x119>
          putc(fd, *s);
 547:	0f be d2             	movsbl %dl,%edx
 54a:	89 f8                	mov    %edi,%eax
 54c:	e8 50 fe ff ff       	call   3a1 <putc>
          s++;
 551:	83 c6 01             	add    $0x1,%esi
        while(*s != 0){
 554:	0f b6 16             	movzbl (%esi),%edx
 557:	84 d2                	test   %dl,%dl
 559:	75 ec                	jne    547 <printf+0x10c>
      state = 0;
 55b:	be 00 00 00 00       	mov    $0x0,%esi
 560:	e9 02 ff ff ff       	jmp    467 <printf+0x2c>
 565:	8b 7d 08             	mov    0x8(%ebp),%edi
 568:	eb ea                	jmp    554 <printf+0x119>
        putc(fd, *ap);
 56a:	8b 7d e4             	mov    -0x1c(%ebp),%edi
 56d:	0f be 17             	movsbl (%edi),%edx
 570:	8b 45 08             	mov    0x8(%ebp),%eax
 573:	e8 29 fe ff ff       	call   3a1 <putc>
        ap++;
 578:	83 c7 04             	add    $0x4,%edi
 57b:	89 7d e4             	mov    %edi,-0x1c(%ebp)
      state = 0;
 57e:	be 00 00 00 00       	mov    $0x0,%esi
 583:	e9 df fe ff ff       	jmp    467 <printf+0x2c>
        putc(fd, c);
 588:	89 fa                	mov    %edi,%edx
 58a:	8b 45 08             	mov    0x8(%ebp),%eax
 58d:	e8 0f fe ff ff       	call   3a1 <putc>
      state = 0;
 592:	be 00 00 00 00       	mov    $0x0,%esi
 597:	e9 cb fe ff ff       	jmp    467 <printf+0x2c>
    }
  }
}
...
free(void *ap)
{
 ...
 5b0:	a1 20 0a 00 00       	mov    0xa20,%eax
 ...
 5ec:	89 08                	mov    %ecx,(%eax)
  freep = p;
 5ee:	a3 20 0a 00 00       	mov    %eax,0xa20
}
 5f3:	5b                   	pop    %ebx
 5f4:	5e                   	pop    %esi
 5f5:	5f                   	pop    %edi
...
morecore(uint nu)
{
 ...
 633:	50                   	push   %eax
 634:	e8 48 fd ff ff       	call   381 <sbrk>
  if(p == (char*)-1)
 ...
 64b:	e8 54 ff ff ff       	call   5a4 <free>
  return freep;
 650:	a1 20 0a 00 00       	mov    0xa20,%eax
...
malloc(uint nbytes)
{
 ...
 6ac:	74 1a                	je     6c8 <malloc+0x64>
        prevp->s.ptr = p->s.ptr;
      else {
        p->s.size -= nunits;
 6ae:	29 da                	sub    %ebx,%edx
 6b0:	89 50 04             	mov    %edx,0x4(%eax)
        p += p->s.size;
 6b3:	8d 04 d0             	lea    (%eax,%edx,8),%eax
        p->s.size = nunits;
 6b6:	89 58 04             	mov    %ebx,0x4(%eax)
      }
      freep = prevp;
 6b9:	89 0d 20 0a 00 00    	mov    %ecx,0xa20
      return (void*)(p + 1);
 ...
 6d7:	73 d1                	jae    6aa <malloc+0x46>
    if(p == freep)
 6d9:	39 05 20 0a 00 00    	cmp    %eax,0xa20
 6df:	75 ed                	jne    6ce <malloc+0x6a>
      if((p = morecore(nunits)) == 0)
 6e1:	89 d8                	mov    %ebx,%eax
 6e3:	e8 2c ff ff ff       	call   614 <morecore>
 6e8:	85 c0                	test   %eax,%eax
 6ea:	75 e2                	jne    6ce <malloc+0x6a>
        return 0;
 6ec:	b8 00 00 00 00       	mov    $0x0,%eax
 6f1:	eb cf                	jmp    6c2 <malloc+0x5e>
